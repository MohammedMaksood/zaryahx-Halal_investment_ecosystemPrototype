...
import { Bell, Menu, User, X, LogIn, Wallet, BarChart3, ChevronDown, Calculator, DollarSign } from 'lucide-react';
...
// Notification interface removed and converted to comment
/**
 ...
 * @property {'transaction' | 'system' | 'alert'} type
 */
...
export const Navbar = () => {
            ...
            {sortedNotifications.length > 0 ? (
              sortedNotifications.map(notif => (
                <div key={notif.id} className="p-2 rounded bg-white/5 hover:bg-white/10 transition-colors">
                  ...
                  <div className="text-sm text-black/70">{notif.message}</div>
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-lavender/10 backdrop-blur-md border-b border-lavender/30">
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            <Link to="/stocks" className="text-sm font-medium text-black hover:text-lavender transition-colors">
              Halal Stocks
            </Link>
            <Link to="/analysis" className="text-sm font-medium text-black hover:text-lavender transition-colors">
              Analysis
            </Link>
            <Link to="/portfolio" className="text-sm font-medium text-black hover:text-lavender transition-colors">
              Portfolio
            </Link>
            <Link to="/islamic-finance" className="text-sm font-medium text-black hover:text-lavender transition-colors">
              Islamic Finance
            </Link>
            <Link to="/zakat-calculator" className="text-sm font-medium text-black hover:text-lavender transition-colors">
              Zakat Calculator
            </Link>
            {/* Islamic Finance Advisor link removed */}
            {/* Unified Lifestyle button removed */}
          </nav>
                  ...
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="hover:bg-lavender/20 rounded-full h-8 w-8 p-0">
                      <Avatar className="h-8 w-8">
                        ...
                        <AvatarFallback className="bg-lavender text-cream text-xs">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden glassy-card absolute top-16 left-0 right-0 border-b border-t border-white/10 animate-fade-in">
          <div className="space-y-1 px-4 py-5">
            ...
            {/* Islamic Finance Advisor mobile link removed */}
            {/* Unified Lifestyle mobile button removed */}
                  ...
                  <>
                    <div className="flex items-center space-x-3 px-3 py-2">
                      <Avatar className="h-10 w-10">
                        <AvatarImage src={user?.profileImage} alt={user?.name || 'User'} />
                        <AvatarFallback className="bg-lavender text-cream">
                          {user?.name ? getInitials(user.name) : 'U'}
                        </AvatarFallback>
                      </Avatar>
                      <div>
