...
const OrderForm = ({
  ...
  /**
   * @type {[string, function(string): void]} - Order type: 'buy' or 'sell'
   */
  const [orderType, setOrderType] = useState('buy');
  const [quantity, setQuantity] = useState('1');
  
  /**
   * @type {[string, function(string): void]} - Exchange: 'NSE' or 'BSE'
   */
  const [exchange, setExchange] = useState('NSE');
  
  /**
   * @type {[string, function(string): void]} - Order mode: 'market' or 'limit'
   */
  const [orderMode, setOrderMode] = useState('market');
  const [limitPrice, setLimitPrice] = useState(currentPrice ? currentPrice.toFixed(2) : '0.00');
  const [useStopLoss, setUseStopLoss] = useState(true);
  const [stopLossPrice, setStopLossPrice] = useState(currentPrice ? (currentPrice * 0.95).toFixed(2) : '0.00');
  const [useAI, setUseAI] = useState(true);
  const [aiRiskLevel, setAiRiskLevel] = useState(50);
  const [estimatedTotal, setEstimatedTotal] = useState(currentPrice || 0);
  // useState<{
  ...
  const handleAiRiskLevelChange = (value) => {
    ...
    <div className="order-form space-y-6">
      <Tabs defaultValue="buy" className="w-full">
        ...
        <TabsContent value="buy" className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={exchange} onValueChange={(value) => setExchange(value)}>
          ...
          <div className="space-y-2">
            ...
            <RadioGroup defaultValue="market" className="flex space-x-4" onValueChange={(value) => setOrderMode(value)}>
              ...
              {useStopLoss && (
                <div className="flex items-start space-x-2 text-xs">
                  ...
                  <p className="text-white/70">
                    AI h a stop loss at ${aiPrediction.stopLossRecommendation.toFixed(2)} based on 
        ...
        <TabsContent value="sell" className="space-y-4">
          {/* Sell tab h same fields but with sell-specific logic */}
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
