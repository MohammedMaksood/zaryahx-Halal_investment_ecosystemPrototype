...
const Wallet = () => {
  ...
  /**
   * @type {Array<{id: string, type: string, amount: number, date: string, status: string, method: string}>}
   */
  const [transactions, setTransactions] = useState([]);
  /**
   * @type {Array<{id: string, name: string, value: number, allocation: number}>}
   */
  ...
  useEffect(() => {
    if (buyAction && symbol && price) {
      /**
       * @type {Object.<string, string>} - Mapping of stock symbols to their names
       */
      const stockNames = {
      ...
      setTimeout(() => {
        if (balance === 0) {
          ...
          toast({
            ...
            description: `Enter the number of shares you want to buy for ${stockName} at $${price ? price.toFixed(2) : '0.00'}`,
          });
  ...
  const handleDeposit = () => {
    ...
    toast({
      ...
      description: `$${amount.toFixed(2)} h added to your wallet`,
    });
  };
  ...
  const handleWithdraw = () => {
    ...
    toast({
      ...
      description: `$${amount.toFixed(2)} h withdrawn from your wallet`,
    });
  };
