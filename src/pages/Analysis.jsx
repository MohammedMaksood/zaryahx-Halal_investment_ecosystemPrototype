...
import { getShariahCompliance } from "@/services/zoyaApi";
...
const Analysis = () => {
  ...
  const [searchType, setSearchType] = useState('symbol'); // 'symbol' | 'name'
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [analysisComplete, setAnalysisComplete] = useState(false);
  const [analysisResult, setAnalysisResult] = useState(null); // 'halal' | 'haram' | null
   ...
   * @property {number} complianceScore - Shariah compliance score
   * @property {number} debtRatio - Debt to assets ratio
   * @property {number} interestIncome - Interest income percentage
   * @property {number} illiquidAssets - Illiquid assets percentage
   * @property {number} haramRevenue - Haram revenue percentage
   */
  ...
  // List of terms that should always be classified 
  const haramTerms = [
  ...
  const evaluateShariah = (query) => {
    query = query.toLowerCase();
    
    // STRICT CHECK: Direct match with haram terms - automatic fail
    // This ensures alcohol companies are always marked 
    ...
    // Default case - random with bi halal
  ...
  const alcoholCompanies = [
    ...
    // Generic alcohol terms that should always be identified -compliant
  ...
  const gamblingCompanies = [
    { name: 'L Sands', symbol: 'LVS', industry: 'Gambling' },
  ...
  /**
   * @param {React.FormEvent} [e] - The form event (optional)
   */
  const handleSearch = async (e) => {
    // Only call preventDefault if e exists
  ...
  const getReasonsList = (isHalal, stockInfo) => {
    if (!stockInfo) return [];
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <form onSubmit={handleSearch} className="relative max-w-md mx-auto">
            <div className="flex mb-3">
              <Button
                ...
                className={`rounded-r-none ${searchType === 'symbol' ? 'bg-lavender text-white' : 'text-black'}`}
              ...
              <Button
                ...
                className={`rounded-l-none ${searchType === 'name' ? 'bg-lavender text-white' : 'text-black'}`}
          ...
          {recentSearches.length > 0 && (
            <div className="mt-4">
              <p className="text-xs text-black mb-2">Recent searches:</p>
                ...
                {recentSearches.map((search, index) => (
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      // Call handleSearch without an event parameter since we're not in a form submit
                      handleSearch();
                    }}
                  >
                    {search}
          ...
          <div className="rounded-xl glassy-card p-8">
            <div className="text-center py-12">
              ...
              <div className="max-w-md mx-auto mt-12">
                ...
                <div className="mt-8 grid grid-cols-2 gap-4 text-left text-xs text-black">
            ...
            <div className="flex flex-col lg:flex-row justify-between">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <HalalBadge type={analysisResult} />
                </div>
                <div className="text-black text-sm mb-2">{stockInfo?.symbol} • {stockInfo?.sector}</div>
              ...
              <div className="mt-4 lg:mt-0">
                ...
                <div className="text-xs text-black mt-1">Last updated: {new Date().toLocaleDateString()}</div>
              </div>
            </div>
            ...
            <div className="my-6 border-t border-b border-white/10 py-6">
              ...
              <div className="mb-8 relative">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-black">Shariah Compliance Score</span>
                  <div className="flex items-center">
                    <span className="font-semibold text-lg">{stockInfo?.complianceScore}</span>
                    <span className="text-xs text-black ml-1">/100</span>
                  </div>
                </div>
                ...
                <div className="h-4 w-full bg-white/10 rounded-full overflow-hidden relative">
                  ...
                  <div className="absolute top-full left-[33%] text-[10px] text-black mt-3 -ml-6">Concern</div>
                  <div className="absolute top-full left-[66%] text-[10px] text-black mt-3 -ml-6">Acceptable</div>
                  <div className="absolute top-full left-[90%] text-[10px] text-black mt-3 -ml-6">Excellent</div>
                </div>
              </div>
            </div>
            ...
            <div className="max-w-3xl mx-auto">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="glassy-card rounded-xl p-6">
                  ...
                  <ul className="space-y-3">
                    <li className="flex items-start">
                      ...
                      <p className="text-black">Total debt divided by total assets must be less than 33%</p>
                    </li>
                    <li className="flex items-start">
                      <div className="h-5 w-5 rounded-full bg-lavender/20 flex items-center justify-center mt-0.5 mr-3">
                        <span className="text-xs text-lavender">2</span>
                      </div>
                      <p className="text-black">Total interest-bearing securities divided by market cap must be less than 33%</p>
                    </li>
                    <li className="flex items-start">
                      <div className="h-5 w-5 rounded-full bg-lavender/20 flex items-center justify-center mt-0.5 mr-3">
                        <span className="text-xs text-lavender">3</span>
                      </div>
                      <p className="text-black">Accounts receivable divided by total assets must be less than 33%</p>
                    </li>
                    <li className="flex items-start">
                      <div className="h-5 w-5 rounded-full bg-lavender/20 flex items-center justify-center mt-0.5 mr-3">
                        <span className="text-xs text-lavender">4</span>
                      </div>
                      <p className="text-black">Non-permissible income must be less than 5% of total revenue</p>
                    </li>
                  </ul>
                </div>
                ...
                <div className="glassy-card rounded-xl p-6">
                  ...
                  <p className="mb-4 text-black">Companies are excluded if they are involved in:</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex items-center space-x-2">
                      <div className="h-2 w-2 rounded-full bg-haram"></div>
                      <span className="text-sm text-black">Alcohol</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="h-2 w-2 rounded-full bg-haram"></div>
                      <span className="text-sm text-black">Tobacco</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="h-2 w-2 rounded-full bg-haram"></div>
                      <span className="text-sm text-black">Gambling</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="h-2 w-2 rounded-full bg-haram"></div>
                      <span className="text-sm text-black">Adult Entertainment</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="h-2 w-2 rounded-full bg-haram"></div>
                      <span className="text-sm text-black">Pork Products</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="h-2 w-2 rounded-full bg-haram"></div>
                      <span className="text-sm text-black">Weapons</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="h-2 w-2 rounded-full bg-haram"></div>
                      <span className="text-sm text-black">Conventional Banking</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="h-2 w-2 rounded-full bg-haram"></div>
                      <span className="text-sm text-black">Conventional Insurance</span>
                    </div>
                  </div>
                </div>
              </div>
