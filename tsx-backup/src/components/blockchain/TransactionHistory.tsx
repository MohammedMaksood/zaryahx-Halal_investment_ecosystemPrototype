...
// Transaction types
type TransactionType = 'all' | 'sent' | 'received' | 'swap' | 'approve';

// Transaction status
type TransactionStatus = 'confirmed' | 'pending' | 'failed';

// Shariah compliance status
type ComplianceStatus = 'compliant' | 'questionable' | 'non-compliant';
...
const sampleTransactions: Transaction[] = [
  ...
  },
  {
    ...
    complianceDetails: {
      score: 75,
      issues: ['Destination token has mixed sources of income'],
    }
  },
  {
    id: '4',
...
const TransactionHistory: React.FC = () => {
  ...
  return (
    <Card className="bg-secondary/30 border-white/10">
      ...
      <CardContent>
        <Tabs defaultValue="all" value={activeTab} onValueChange={(value) => setActiveTab(value as TransactionType)}>
                    ...
                    <div className={`mt-2 p-2 rounded-md flex items-start gap-2 text-sm ${
                      ...
                      <div>
                        <div className="font-medium capitalize">
                          ...
                          {tx.complianceDetails && tx.complianceDetails.score > 0 && (
                            <span className="ml-1">({tx.complianceDetails.score}/100)</span>
                          )}
                        </div>
                        {tx.complianceDetails && tx.complianceDetails.issues.length > 0 && (
