...
const CommunityPortfolio: React.FC = () => {
  ...
  const [userContributions, setUserContributions] = useState<{[key: number]: string}>({});
  const [isLoading, setIsLoading] = useState(false);
  const [transactionStatus, setTransactionStatus] = useState<'pending' | 'success' | 'error' | null>(null);
  const [animationType, setAnimationType] = useState<'transaction' | 'connection' | 'verification'>('transaction');
  ...
  const loadUserContributions = async () => {
    ...
    try {
      ...
      const contributionMap: {[key: number]: string} = {};
  ...
  const handleCreatePortfolio = async (params: CreatePortfolioParams) => {
    if (!isConnected) {
      toast({
        title: 'Not connected',
  ...
  const handleContribute = async (portfolioId: number, amount: string) => {
    if (!isConnected) {
      toast({
        title: 'Not connected',
        description: 'Please connect your wallet first',
  ...
  const handleProposeInvestment = async (params: ProposeInvestmentParams) => {
    if (!isConnected) {
      toast({
        title: 'Not connected',
        description: 'Please connect your wallet first',
  ...
  const handleVote = async (portfolioId: number, proposalId: number, support: boolean) => {
    if (!isConnected) {
      toast({
        title: 'Not connected',
        description: 'Please connect your wallet first',
  ...
  const handleExecuteInvestment = async (portfolioId: number, proposalId: number) => {
    if (!isConnected) {
      toast({
        title: 'Not connected',
        description: 'Please connect your wallet first',
